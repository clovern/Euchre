...
class Player:
    ...
    def player_is_dealer(self, i, player_is_dealer = "no"):
        ...
        column, row = 0, 2
        counter = 1
        clb_1, clb_2, clb_3, clb_4, clb_5 = Button(window), Button(window), Button(window), Button(window), Button(window)
        buttons_list = [clb_1, clb_2, clb_3, clb_4, clb_5]
        if player != None:
            tell_hand_label = Label(text = "The cards in your hand are: \n").grid(column = column, row = row)
        for item in deck:
            n = deck.index(item)
            card_picture = item.get_pic()
            buttons_list[n].configure(image = card_picture, command = self.dealer_removal_choice)
            buttons_list[n].grid(column = counter, row = 4)
            counter += 1
    
    def dealer_removal_choice(self):
        print("in dealer removal choice")
    ...
    def dealer_removal_assign():
        print("in dealer removal assign")
...
class AI:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck):
        ...
        if pick_up_decision == "yes":
            ...
            window.after(1000, lambda: Player.player_is_dealer(self, i))
        else:
