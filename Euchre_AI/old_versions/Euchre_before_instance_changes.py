...
class Window(Tk):		
    ...
    def done_entering_names_command(self):
        ...
        self.after(5000, lambda:[self.delete_window_widgets(), euchre.euchre_greeting_2(euchre)
...
class Euchre(Card_game):
    ...
    def euchre_shuffle(self):
        ...
        dealing_label = ttk.Label(self, text = "{} is dealing the cards...".format(self.dealer.name)).pack()
        window.after(3000, self.delete_window_widgets())
            # print("\n\n{} is dealing the cards...".format(self.dealer.name))
            # round_number = ["1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", "10th", "11th", "12th", "13th", "14th" \
                           # "15th", "16th", "17th", "18th", "19th"]
            # print("\n\nWe are now beginning the {} round of Euchre.\n".format(round_number[self.counter]))
            # self.print_spaces(3)
            # input("(Press Enter)\n\n")
            
            # trump_suit, trump_card = Euchre.trump_suit_card.get_suit(), Euchre.trump_suit_card.get_card_name()
            # dealer_index = Euchre.players.index(self.dealer)
            # if dealer_index == 4:
                # first_caller = (Euchre.players[0]).name
            # else:
                # first_caller = Euchre.Player_1 if self.dealer == Euchre.Player_4 else Euchre.players[(dealer_index+1)]
            # player_order, deck_order = self.euchre_player_order(first_caller)
            # trump_info_list = self.pick_up(trump_card, trump_suit, self.dealer, dealer_index, player_order, deck_order)
