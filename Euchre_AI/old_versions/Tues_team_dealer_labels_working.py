...
class Window(Tk):		
    ...
    def print_entered_teams(self, event):
        Label5 = ttk.Label(self, text = self.player1name.get()).place(relx = 0, rely = 0)
        self.name_entry1.delete(0, END)
        
    
        
    def titleframe_encase(self):
        greeting_pt1 = ttk.Label(window, text = "Hello, and Welcome to Euchre!\n\n").grid(column = 0, row = 0, sticky = N)
        greeting_pt2 = ttk.Label(window, text = "Please enter your name").grid(column = 0, row = 1, sticky = N)
        window.titleframe_encase()
        self.player_name = StringVar()
        self.name_entry1 = Entry(self, width = 20, textvariable = self.player_name)
        self.name_entry1.grid(in_ = self.team1frame, column =0, row = 3, sticky = N)
        done_enter_names_button = ttk.Button(window, text = "Done Entering Names", command = self.done_entering_names_command).grid(column = 0, row = 4, sticky = N)
