...
class Euchre(Card_game):
    ...
    def pick_up_scene(self):
        ...
        for person in self.player_order:
            if person != game_1.human_player:
                ...
                column_counter += 1
        missing_AI_name = self.player_order[0].name if self.player_order[0] != self.human_player else self.player_order[1].name
            
        AI2_pass_pickup_name = Label(window, text = missing_AI_name)
        AI2_pass_pickup_name.grid(column= 2, row = 7)
    ...
    def call_it(self, i = 0):
        ...
        self.instructions_label.place(relx = 0, rely = 0)
        column_counter = 2
...
class AI:
    ...
    def three_cards_trump(self, trumpst_card_names, unique_offsuits_num, off_trump_card_names, dealer_on_team, trump_card, dec_num):
        ...
        if (comp_deck_ct_1 == 3):
            ...
            #print("3")
            loner_player, pick_up_decision = ("no", "yes") if (dealer_on_team == "yes") else (("no", "no") if (unique_offsuits_num != 1 and ("ace" not in off_trump_card_names)) else (("no", "yes") if dec_num <= .85 else ("no", "no")))
    ...
    def two_cards_trump(self, trumpst_card_names, unique_offsuits_num, off_trump_card_names, dealer_on_team, trump_card, dec_num):
        ...
        if comp_deck_ct_1 == 2:
            ...
            if dealer_on_team == "no":
                if unique_offsuits_num == 1 or "ace" in off_trump_card_names:
                    pick_up_decision = "yes" if dec_num < .65 else "no"
                else:
                    pick_up_decision = "yes" if (trump_card not in ["jack", "jack_lft"] and unique_offsuits_num != 3 and dec_num < .3)  else ("yes" if dec_num < .15 else "no")
            ...
            if dealer_on_team == "yes":
                ...
                if comp_deck_ct_2 == 1:
                    if comp_deck_ct_3 == 0:
                        pick_up_decision = "yes" if dec_num < .75 else "no"
                    else:
                        pick_up_decision = "yes" if dec_num < .4 else "no"
                else:
                    pick_up_decision = "yes" if dec_num < .05 else "no"
            if dealer_on_team == "no":
                #print("off_trump_card_names.count(ace) = " + str(off_trump_card_names.count("ace")))
                if comp_deck_ct_2 == 1 and off_trump_card_names.count("ace") >= 2:
                    #print("11")
                    pick_up_decision = "yes"
                if comp_deck_ct_2 == 1 and off_trump_card_names.count("ace") == 1 or (unique_offsuits_num == 1 and comp_deck_ct_3 < 1):
                    #print("12")
                    pick_up_decision = "yes" if dec_num < .38 else "no" 
                else:
                    pick_up_decision = "no"
        #print("comp decks")
        #print(comp_deck_ct_1, comp_deck_ct_2, comp_deck_ct_3, comp_deck_ct_4, comp_deck_ct_5, comp_deck_ct_6)
                ...
                if choosing_deck[highest_card_index] == "jack_lft":
                    for card in player_deck:
                        if card == left_bower:
                            potential_card = card
                for card in player_deck:
                    print("card name, card suit, choosing deck[highest card index], trump_suit = ")
                    print(card.get_card_name(), card.get_suit())
                    print(choosing_deck[highest_card_index], game_1.trump_suit)
            ...
            if current_winning_player == Euchre.player_opposites[player]:
                if i == (len(game_1.player_order) - 1):
                    ...
                    if potential_laid_down[potential_winner_index] == potential_card:
                        ...
                        dec_num = random.random()
                        if current_winning_card.get_value() <= 11:
                            if dec_num < .65:
                                if other_suits_in_hand_card:
