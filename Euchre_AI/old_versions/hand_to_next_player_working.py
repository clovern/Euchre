...
class Card_game:
    ...
    def tell_hand(self, deck, player = None):
        ...
        for item in deck:
            ...
            card_label = Label(image = card_picture).grid(column = counter, row = 2)
            counter += 1
        ...
        if i >= 4:
            ...
            for person in self.player_order:
                if person != player:
                    ...
                    player_response = Label(window, text = response_text).grid(column=column_counter, row = 10)
                    column_counter += 1
            player.pick_up_questions_2(i, self.trump_card, self.trump_suit, self.dealer, player, \
                                                             self.dealer_index, player_deck, player_responses)  
...
class Player:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck, player_responses):
        print("in pick_up_questions_1")
        Card_game.tell_hand(self, player_deck, player)
        # pick_up_string, loner_player = None, "no"
        # player_picked_up = None
        pick_up_button = ttk.Button(window, text = "Pick up", command = lambda: self.pick_up_decision(i, player_responses, "no")).grid(column = 2, row = 4)
        loner_button = ttk.Button(window, text = "Pick up and Go Alone", command = lambda: self.pick_up_decision(i, player_responses, game_1.player_order[i])).grid(column = 3, row = 4)
        pass_button = ttk.Button(window, text = "Pass", command = lambda: self.pass_decision(i, player_responses)).grid(column = 4, row = 4)
        window.empty_column_label(7, num_column_spaces = 3)
        potential_trump_frame = ttk.LabelFrame(window, text = "Potential trump")
        potential_trump_frame.grid(column=10, row = 8)
        potential_trump_label = ttk.Label(potential_trump_frame, image = Euchre.trump_suit_card.get_pic())
        potential_trump_label.grid(column = 0, row = 0)
        # if pick_up == "yes":
            # player_picked_up = player
...
your hand to discard. Please enter the number of your chosen card here: ".format(trump_card, trump_suit)
    ...
    def pick_up_questions_2(self, i, trump_card, trump_suit, dealer, dealer_index, player, player_deck, player_responses):           
        ...
        self.call_it_button.grid(column = 2, row = 4)
    ...
    def call_it_decision_1(self, i, player_responses, loner_decision):
        ...
        if loner_decision == "yes":
            call_spades_button = ttk.Button(window, text = "call spades", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "spades")).grid(column = 2, row = 4)
            call_clubs_button = ttk.Button(window, text = "call clubs", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "clubs")).grid(column = 3, row = 4)
            call_diamonds_button = ttk.Button(window, text = "call diamonds", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "diamonds")).grid(column = 4, row = 4)
            call_hearts_button = ttk.Button(window, text = "call hearts", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "hearts")).grid(column = 5, row = 4)
        else:
            call_spades_button = ttk.Button(window, text = "call spades", command = lambda: self.call_it_decision_2(i, player_responses, "no", "spades")).grid(column = 2, row = 4)
            call_clubs_button = ttk.Button(window, text = "call clubs", command = lambda: self.call_it_decision_2(i, player_responses, "no", "clubs")).grid(column = 3, row = 4)
            call_diamonds_button = ttk.Button(window, text = "call diamonds", command = lambda: self.call_it_decision_2(i, player_responses, "no", "diamonds")).grid(column = 4, row = 4)
            call_hearts_button = ttk.Button(window, text = "call hearts", command = lambda: self.call_it_decision_2(i, player_responses, "no", "hearts")).grid(column = 5, row = 4)
...
class AI:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck, player_respones):
        #testprint
