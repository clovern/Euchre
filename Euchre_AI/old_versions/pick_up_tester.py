...
class Window(Tk):		
    ...
    def done_entering_names_command(self):
        ...
        self.after(100, lambda:[self.delete_window_widgets(), game_1.euchre_greeting_2()])
...
class Euchre(Card_game):
    ...
    def set_dealer(self):
        ...
        window.after(100, lambda: [window.delete_window_widgets(), self.set_orders()])
    ...
    def pick_up_scene(self):
        ...
        self.loner_button = ttk.Button(window, text = "Pick up and Go Alone", state = "disabled", command = lambda: self.human_playerpick_up_decision(self.human_player_index, game_1.player_order[i]))
...
class Player:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck):
        self.instructions_label =  Label(window, text = "{}, will you tell the dealer to pick it up?\n\n".format(player.name))
        self.instructions_label.place(relx=0, rely=(0), anchor=NW)
...
your hand to discard. Please enter the number of your chosen card here: ".format(trump_card, trump_suit)
    ...
    def pick_up_questions_2(self, i, trump_card, trump_suit, dealer, dealer_index, player, player_deck):
        ...
        self.call_it_button = ttk.Button(window, text = "call a suit", command = lambda: self.call_it_decision_1(i, "no"))
        self.call_it_button.grid(column = 2, row = 5)
        self.loner_button = ttk.Button(window, text = "call a suit \nAND go alone", command = lambda: self.call_it_decision_1(i, "yes"))
        self.loner_button.grid(column = 3, row = 5)
        self.pass_button = ttk.Button(window, text = "Pass", command = lambda: self.pass_decision_callround(i))
        self.pass_button.grid(column = 4, row = 5)
        if i == 3:
    ...
    def call_it_decision_1(self, i,loner_decision):
        ...
        if loner_decision == "yes":
            call_spades_button = ttk.Button(window, text = "call spades", command = lambda: self.call_it_decision_2(i, game_1.player_order[i], "spades")).grid(column = 2, row = 5)
            call_clubs_button = ttk.Button(window, text = "call clubs", command = lambda: self.call_it_decision_2(i, game_1.player_order[i], "clubs")).grid(column = 3, row = 5)
            call_diamonds_button = ttk.Button(window, text = "call diamonds", command = lambda: self.call_it_decision_2(i, game_1.player_order[i], "diamonds")).grid(column = 4, row = 5)
            call_hearts_button = ttk.Button(window, text = "call hearts", command = lambda: self.call_it_decision_2(i, game_1.player_order[i], "hearts")).grid(column = 5, row = 5)
        else:
...
class AI:
    ...
    def pass_decision(self, i):
        print("in pass decision")
        #window.after(1000, game_1.player_response_label_list[i].configure(text = "deciding"))
        window.update()
