...
class Euchre(Card_game):
    ...
    def euchre_round_set_scene_pt_1(self):
        ...
        t2_players_label.grid(column = 0, row = 3)
        team_picked_up_string = "Team 1" if self.team_picked_up == Euchre.team_1 else "Team 2"
        call_or_pickup_string = "Called" if self.dealer_pickup_yn == "no" else "Picked up"
        ...
        team_picked_up_info_label = Label(self.round_info_labelframe, text = "{0}".format(team_picked_up_string))
    ...
    def euchre_round_set_scene_pt_2(self):
        ...
        self.team_tricks_won_label.place(relx = .32, rely = .9)
    ...
    def clear_for_next_trick(self):
        ...
        cards_played_pic_list = []
        if self.human_player.card_play_lf:
            cards_played_pic_list.extend([self.human_player.card_play_lf, self.human_player.card_play_label])
        if Euchre.Player_2.card_play_lf:
            cards_played_pic_list.extend([Euchre.Player_2.card_play_lf, Euchre.Player_2.card_play_label])
        if Euchre.Player_3.card_play_lf:
            cards_played_pic_list.extend([Euchre.Player_3.card_play_lf, Euchre.Player_3.card_play_label])
        if Euchre.Player_4.card_play_lf:
            cards_played_pic_list.extend([Euchre.Player_4.card_play_lf, Euchre.Player_4.card_play_label])
    ...
    def winner_announcement(self):
        ...
        game_winner_frame.place(relx = .4, rely = .34)
        label_text = ("arial", 16, "bold")
        
        if self.team_1_score >= 10:
            game_winner_label = ttk.Label(game_winner_frame, text = "Congratulations, you won the game!")
        else:
            game_winner_label = ttk.Label(game_winner_frame, text = "Bummer, the other team won the game :(\n                Better luck next time!")
        game_winner_label.grid(column = 0, row = 0)
        
        self.next_trick_button = Button(window, text = "Play again", command = lambda: [window.delete_window_widgets(), window.titleframe_encase()])
        self.next_trick_button.place(relx = .44, rely = .55)
    ...
    def pick_up_scene(self):
        ...
        for person in self.player_order:
            if person != game_1.human_player:
                ...
                column_counter += 1
        AI2_pass_pickup_name = Label(window, text = "AI 2")
...
class Player:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck):
        ...
        if game_1.human_player == dealer:
            game_1.instructions_label.configure(text = " {}, will you pick it up?".format(player.name), fg = "blue")
        else:
...
class AI:
    ...
    def three_cards_trump(self, trumpst_card_names, unique_offsuits_num, off_trump_card_names, dealer_on_team, trump_card, dec_num):
        ...
        if (comp_deck_ct_1 == 3):
            ...
            print("1.5")
            if comp_deck_ct_2 == 2 or (comp_deck_ct_2 == 1 and unique_offsuits_num == 1):
                loner_player, pick_up_decision = "yes", "yes"
            else:
                pick_up_decision = "yes"
        ...
        if comp_deck_ct_1 == 2:
            ...
            if dealer_on_team == "no":
                if unique_offsuits_num == 1 or "ace" in off_trump_card_names:
                    ...
                    pick_up_decision = "yes" if (trump_card not in ["jack", "jack_lft"] and unique_offsuits_num != 3 and dec_num < .4)  else ("yes" if dec_num < .15 else "no")
    ...
    def pick_up_questions_2(self, i, trump_card, trump_suit, dealer, dealer_index, player, player_deck):
        ...
        for list in all_suits_nested_list:
            ...
            partner_list = None
            for list in all_suits_nested_list:
                if list[0] == list_suit_pair:
                    partner_list = list
            if partner_list:
                if "jack" in partner_list:
                    list.append("jack_lft")
            other_suits_in_deck = len(all_suits_nested_list)
            other_cards_in_deck = []
            for list2 in all_suits_nested_list:
                if list2 != list:
                    other_cards_in_deck.extend(list2[1:])
            ...
            if len(list) - 1 == 5:
                ...
                loner_player, called_suit = AI.call_suit_4_card(self, list, dec_num)
            elif len(list)-1 == 3:
                loner_player, called_suit = AI.call_suit_3_card(self, list, other_suits_in_deck, other_cards_in_deck, dec_num)
            elif len(list)-1 ==2:
