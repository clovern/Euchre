...
class Window(Tk):		
    ...
    def titleframe_encase(self):
        ...
        self.name_entry2.insert(END, "AI 3")
        ...
        self.name_entry3.insert(END, "AI 2")
    ...
    def done_entering_names_command(self):
        ...
        self.after(3000, lambda:[self.delete_window_widgets(), game_1.euchre_greeting_2()])
...
class Euchre(Card_game):
    ...
    def euchre_greeting(self):
        ...
        done_enter_names_button = ttk.Button(window, text = "Done Entering Names", command = window.done_entering_names_command).grid(column = 1, row = 10)


    def euchre_greeting_2(self):
        Euchre.Player_1 = AI(window.player1name, AI.player_1_icon) if window.player1name[:2] == "AI" else Player(window.player1name, Player.player_1_icon)
        Euchre.Player_2 = AI(window.player2name, AI.player_2_icon) if window.player2name[:2] == "AI" else Player(window.player2name, Player.player_2_icon)
        Euchre.Player_3 = AI(window.player3name, AI.player_3_icon) if window.player3name[:2] == "AI" else Player(window.player3name, Player.player_3_icon)
        Euchre.Player_4 = AI(window.player4name, AI.player_4_icon) if window.player4name[:2] == "AI" else Player(window.player4name, Player.player_4_icon)
    ...
    def set_dealer(self):
        ...
        dealing_label = ttk.Label(window, text = "We are now beginning the {0} round of Euchre.\n\n\n {1} is dealing the cards...".format(round_number[self.counter], self.dealer.name)).grid(column = 0, row = 0)
    ...
    def pick_up(self, i = 0, player_responses = [None, None, None, None]):
        if i >= 4:
            ...
            player = self.player_order[i]
            column_counter = 2
            if self.num_human_players > 1:
                if type(player) == Player:
                    next_player_var = IntVar()
                    hand_to_next_player = Label(window, text = "Please hand the computer to {}. Press the \"Ready\" button when you are ready to go".format(player.name))
                    hand_to_next_player.grid(column = 0, row = 0)
                    self.space_label(0, 1)
                    ready_button = Button(window, text="Ready", command=lambda: [window.delete_window_widgets(), next_player_var.set(1)])
                    ready_button.grid(column = 0, row = 2)
                    ready_button.wait_variable(next_player_var)
            ...
            for person in self.player_order:
                if person != player:
                    ...
                    response_text = "Has not gone \n   yet" if player_responses[n] == None else player_responses[n]
                    player_response = Label(window, text = response_text).grid(column=column_counter, row = 8)
                    column_counter += 1
            player.pick_up_questions_1(i, self.trump_card, self.trump_suit, self.dealer, player, \
                                                             self.dealer_index, player_deck, player_responses)  
        
        
        #trump_info_list =   pick_up questions 1 originally
                                                         
            # if trump_info_list:
                # if type(self.dealer) == AI:
                    # self.dealer.dealer_removal_choice(Euchre.deck_order_standard[dealer_index], trump_suit, trump_card, dealer_index)
                # elif type(self.dealer) == Player:
                    # player_picked_up = player
                    # self.dealer.dealer_removal_choice(dealer_index, self.dealer, player_picked_up, trump_card, trump_suit)
                # return trump_info_list
        
        
    def call_it(self, i = 0, player_responses = [None, None, None, None]):
        print("in call it")
        print("i = " + str(i))
        if i >= 4:
            return
            ...
            for person in self.player_order:
                if person != player:
                    player_pass_pickup_icon = Label(image = person.get_pic()).grid(column = column_counter, row = 8)
                    player_pass_pickup_name = Label(window, text = person.name).grid(column=column_counter, row = 9)
                    n = self.player_order.index(person)
                    response_text = "Has not gone \n   yet" if player_responses[n] == None else player_responses[n]
                    player_response = Label(window, text = response_text).grid(column=column_counter, row = 10)
                    column_counter += 1
            print("right before player.pick_up_qs_2")
            player.pick_up_questions_2(i, self.trump_card, self.trump_suit, self.dealer, self.dealer_index, player, \
                                                             player_deck, player_responses)  
...
class Player:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck, player_responses):
        instructions_label = Label(window, text = "{}, will you tell the dealer to pick it up?\n\n".format(player.name)).grid(column = 0, row = 1)
        Card_game.tell_hand(self, player_deck, player = player)
        # pick_up_string, loner_player = None, "no"
        # player_picked_up = None
        pick_up_button = ttk.Button(window, text = "Pick up", command = lambda: self.pick_up_decision(i, player_responses, "no")).grid(column = 2, row = 5)
        loner_button = ttk.Button(window, text = "Pick up and Go Alone", command = lambda: self.pick_up_decision(i, player_responses, game_1.player_order[i])).grid(column = 3, row = 5)
        pass_button = ttk.Button(window, text = "Pass", command = lambda: self.pass_decision(i, player_responses)).grid(column = 4, row = 5)
        window.empty_column_label(7, num_column_spaces = 3)
        potential_trump_frame = ttk.LabelFrame(window, text = "Potential trump")
        potential_trump_frame.grid(column=8, row =6)
        potential_trump_label = ttk.Label(potential_trump_frame, image = Euchre.trump_suit_card.get_pic())
        potential_trump_label.grid(column = 0, row = 0, columnspan = 3)
        # if pick_up == "yes":
            # player_picked_up = player
    ...
    def pick_up_decision(self, i, player_responses, loner_decision):
        print("in pick_up_decision")
        game_1.trump_info_list = ["yes", game_1.player_order[i], game_1.trump_suit, loner_decision]
        game_1.trump_info_list_printer()
    
    def pass_decision(self, i, player_responses):
        print("in pass decision")
        player_responses_copy = player_responses.copy()
        player_responses_copy[i] = "Pass"
        game_1.pick_up(i = i+1, player_responses = player_responses_copy)
...
your hand to discard. Please enter the number of your chosen card here: ".format(trump_card, trump_suit)
    ...
    def pick_up_questions_2(self, i, trump_card, trump_suit, dealer, dealer_index, player, player_deck, player_responses):
        print("player in pick up qs 2 = " + str(player))
        instructions_label = Label(window, text = "{}, would you like to call a suit as trump?\n\n".format(player.name))
        instructions_label.grid(column = 0, row = 1)
        Card_game.tell_hand(self, player_deck, player = player)
        self.call_it_button = ttk.Button(window, text = "call a suit", command = lambda: self.call_it_decision_1(i, player_responses, "no"))
        self.call_it_button.grid(column = 2, row = 5)
        self.loner_button = ttk.Button(window, text = "call a suit \nAND go alone", command = lambda: self.call_it_decision_1(i, player_responses, "yes"))
        self.loner_button.grid(column = 3, row = 5)
        self.pass_button = ttk.Button(window, text = "Pass", command = lambda: self.pass_decision_callround(i, player_responses))
        self.pass_button.grid(column = 4, row = 5)
        if i == 3:
            print("should be disabling button here")
            self.pass_button.config(state = 'disabled')

    def call_it_decision_1(self, i, player_responses, loner_decision):
        print("in call it decision 1")
        self.call_it_button.grid_forget()
        self.loner_button.grid_forget()
        self.pass_button.grid_forget()
        if loner_decision == "yes":
            call_spades_button = ttk.Button(window, text = "call spades", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "spades")).grid(column = 2, row = 5)
            call_clubs_button = ttk.Button(window, text = "call clubs", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "clubs")).grid(column = 3, row = 5)
            call_diamonds_button = ttk.Button(window, text = "call diamonds", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "diamonds")).grid(column = 4, row = 5)
            call_hearts_button = ttk.Button(window, text = "call hearts", command = lambda: self.call_it_decision_2(i, player_responses, game_1.player_order[i], "hearts")).grid(column = 5, row = 5)
        else:
            call_spades_button = ttk.Button(window, text = "call spades", command = lambda: self.call_it_decision_2(i, player_responses, "no", "spades")).grid(column = 2, row = 5)
            call_clubs_button = ttk.Button(window, text = "call clubs", command = lambda: self.call_it_decision_2(i, player_responses, "no", "clubs")).grid(column = 3, row = 5)
            call_diamonds_button = ttk.Button(window, text = "call diamonds", command = lambda: self.call_it_decision_2(i, player_responses, "no", "diamonds")).grid(column = 4, row = 5)
            call_hearts_button = ttk.Button(window, text = "call hearts", command = lambda: self.call_it_decision_2(i, player_responses, "no", "hearts")).grid(column = 5, row = 5)

    
    def call_it_decision_2(self, i, player_responses, loner_player, suit):
        game_1.trump_info_list = ["no", game_1.player_order[i], suit, loner_player]
        game_1.trump_info_list_printer()
       
    
    def pass_decision_callround(self, i, player_responses):
        print("in pass decision callround")
        player_responses_copy = player_responses.copy()
        player_responses_copy[i] = "Pass"
        game_1.call_it(i = i+1, player_responses = player_responses_copy)
...
class AI:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck, player_responses):
        ...
        if pick_up_decision == "yes":
            ...
            self.pass_decision(i, player_responses)
    ...
    def pick_up_questions_2(self, i, trump_card, trump_suit, dealer, dealer_index, player, player_deck, player_responses):
        ...
        if i == 3:
            self.caller_force_choose(player_deck_c, trump_suit)
        ...
        if called_suit != None:
            ...
            print("????????????????")
            self.pass_decision_callround(i, player_responses)
