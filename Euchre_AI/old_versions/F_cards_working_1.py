...
class Window(Tk):		
    ...
    def done_entering_names_command(self):
        ...
        self.after(5000, lambda:[self.delete_window_widgets(), game_1.euchre_greeting_2()])
...
class Euchre(Card_game):
    ...
    def set_dealer(self):
        ...
        window.after(3000, lambda: [window.delete_window_widgets(), self.set_orders()])
    ...
    def set_orders(self):
        ...
        trump_info_list = self.pick_up(trump_card, trump_suit)        
    ...
    def pick_up(self, trump_card, trump_suit, i = 0, player_responses = []):
        if player_responses == []:
            player_responses = [None, None, "Pass", "Pass"]
        player = self.player_order[i]
        column_counter = 2
        ...
        for person in self.player_order:
            if person != player:
                ...
                column_counter += 1
        player.pick_up_questions_1(i, trump_card, trump_suit, self.dealer, player, \
...
class Player:
    ...
    def pick_up_questions_1(self, i, trump_card, trump_suit, dealer, player, dealer_index, player_deck):
        ...
        pick_up_button = ttk.Button(window, text = "Pick up", command = self.picked_up_print).grid(column = 2, row = 4)
        loner_button = ttk.Button(window, text = "Pick up and Go Alone", command = self.picked_up_print).grid(column = 3, row = 4)
        pass_button = ttk.Button(window, text = "Pass", command = self.picked_up_print).grid(column = 4, row = 4)
    ...
    def picked_up_print(self):
        print("player decided to pick up")
